export const applyVisualLanguageFiltering = () => {
    ...
    .forEach((element) => element.style.display = element.dataset.langHidable !== localStorage.langCode ? 'none' : element.dataset.display ?? 'block');
    ...
    .forEach((element) => element.style.display = !availableLanguages.includes(element.dataset.langNot_available) ? 'none' : element.dataset.display ?? 'block')
